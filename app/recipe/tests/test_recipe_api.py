...
def create_recipe(user, **params):
    ...
    # override the defaults with what are provided in the params
...
# authentication is required for getting recipes
...
class PrivateRecipeApiTests(TestCase):
    ...
    # tags created by each users are isolated
    def test_create_recipe_with_existing_tags_by_others(self):
        """Test creating a recipe with existing tag."""
        new_user = create_user(email='user2@example.com', password='test123')
        tag_halal = Tag.objects.create(user=new_user, name='Halal')
        payload = {
            'title': 'Naan',
            'time_minutes': 120,
            'price': Decimal('3.50'),
...
class ImageUploadTests(TestCase):
    ...
    # similar to setUp but running after the test
    def tearDown(self):
    ...
    def test_upload_image(self):
        ...
        # create a temporary image file called NamedTemporaryFile
        # used to test uploading to our endpoint
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            img = Image.new('RGB', (10, 10))
            # save the image to the temporary image file
            img.save(image_file, format='JPEG')
            # seeker goes to the beginning of the file
            # so we can actually upload it
            image_file.seek(0)
